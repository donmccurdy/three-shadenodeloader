...
Shader "Shade/Fake Volume"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
            ...
            o.Albedo = remap(sin((IN.worldPos*normalize(IN.worldNormal))), float2(-1.0, 1.0).x, float2(-1.0, 1.0).y, float2(0.0, 1.0).x, float2(0.0, 1.0).y);
            float temp_3 = snoise3D(((IN.worldPos * _NoiseScale)-float3(_Time.y, 0.0, 0.0)));
            o.Alpha = remap(temp_3, float2(-1.0, 1.0).x, float2(-1.0, 1.0).y, float2(0.0, 1.0).x, float2(0.0, 1.0).y);
        }
        ENDCG
    }
}
