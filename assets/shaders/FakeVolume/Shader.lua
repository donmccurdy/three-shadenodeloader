{
    ...
    pass =
    {
        ...
        [[
            ...
            {
                v.position += (((modelMatrix * vec4(v.normal, 0.0)).xyz * float(gl_InstanceID)) * -0.01);
            }
        ]],
        ...
        [[
            ...
            {
                o.emissive = 1.0;
                o.diffuse = remap(sin((IN.worldPosition*normalize(IN.worldNormal))), vec2(-1.0, 1.0).x, vec2(-1.0, 1.0).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y);
                o.emission = vec3(0.0, 0.0, 0.0);
                float temp_3 = snoise3D(((IN.worldPosition * _noiseScale)-vec3(_time, 0.0, 0.0)));
                o.opacity = remap(temp_3, vec2(-1.0, 1.0).x, vec2(-1.0, 1.0).y, vec2(0.0, 1.0).x, vec2(0.0, 1.0).y);
                if (o.opacity < _cutoff) discard;
            }
        ]]
    }
}
